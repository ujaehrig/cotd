...
def cleanup_old_takeover_log(
    conn: sqlite3.Connection,
    retention_days: int = 365,
    dry_run: bool = False,
) -> int:
    """
    Delete takeover_log records older than retention_days.
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM takeover_log WHERE takeover_date < ?",
            (cutoff_date,),
        )
        count = cursor.fetchone()[0]
        ...
        if dry_run:
            logger.info(
                f"[DRY RUN] Would delete {count} takeover log records "
                f"older than {retention_days} days"
            )
            return count

        cursor.execute(
            "DELETE FROM takeover_log WHERE takeover_date < ?",
            (cutoff_date,),
        )
        deleted = cursor.rowcount
        logger.info(
            f"Cleaned up {deleted} old takeover log records "
            f"(older than {retention_days} days)"
        )
        return deleted

    except sqlite3.Error as e:
        logger.warning(f"Failed to clean up old takeover log: {e}")
        return 0
